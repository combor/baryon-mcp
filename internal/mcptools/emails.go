...
func (p pageInput) toRequest(includeBodies bool) (bridgeclient.PageRequest, error) {
	...
	if includeBodies {
		if p.Limit > maxPreviewLimit {
			return bridgeclient.PageRequest{}, fmt.Errorf("include_bodies returns at most %d messages per page, but limit is %d; lower limit, or take the rest with next_before_uid", maxPreviewLimit, p.Limit)
		}
		// Capping the clamped value covers the limits clampLimit rewrites: an
		// omitted one, and a negative one that would otherwise reach the default.
		limit = min(limit, maxPreviewLimit)
	}
...
type listEmailsInput struct {
	...
	IncludeBodies bool   `json:"include_bodies,omitempty" jsonschema:"also return a shortened body for each message, so a page can be triaged without a get_email per message; caps the page at 10 messages"`
	pageInput
}
...
func fetchPage(ctx context.Context, bridge bridgeclient.Bridge, folder string, crit bridgeclient.SearchCriteria, page pageInput, includeBodies bool) (emailPageOutput, error) {
	if folder == "" {
		return emailPageOutput{}, fmt.Errorf("folder is required")
	}
	req, err := page.toRequest(includeBodies)
	if err != nil {
...
// structuredResult picks the content blocks a tool answers with when its whole
// reply is the structured output. Older peers read only content blocks, so
// leaving Content nil has the SDK serialize it into one for them; empty non-nil
// Content stops it echoing that reply, bodies included, into a redundant text
// block for everyone else. get_email builds its own, because legacy peers need
// its bodies fenced.
func structuredResult(req *mcp.CallToolRequest) *mcp.CallToolResult {
...
func registerListEmails(server *mcp.Server, bridge bridgeclient.Bridge) {
	mcp.AddTool(server, &mcp.Tool{
		Name:        "list_emails",
		Description: "List messages in a folder, newest first, with pagination. Returns envelope summaries (subject, sender, date, message_id, flags) and the folder's uidvalidity. Set include_bodies to add a shortened body to each summary, which triages a page in one call instead of a get_email per message. Page with next_before_uid rather than offset so newly arrived mail cannot shift the results." + untrustedNote,
		...
		out, err := fetchPage(ctx, bridge, in.Folder, bridgeclient.SearchCriteria{UnreadOnly: in.UnreadOnly}, in.pageInput, in.IncludeBodies)
		if err != nil {
...
func registerSearchEmails(server *mcp.Server, bridge bridgeclient.Bridge) {
	mcp.AddTool(server, &mcp.Tool{
		Name:        "search_emails",
		Description: "Search messages in a folder by text, sender, recipient, subject, date range, or unread state. Returns envelope summaries newest first, with pagination, and with include_bodies a shortened body for each match. Search the All Mail folder to look across the whole mailbox at once." + untrustedNote,
		...
		out, err := fetchPage(ctx, bridge, in.Folder, crit, in.pageInput, in.IncludeBodies)
		if err != nil {
			return nil, emailPageOutput{}, err
		}
		return structuredResult(req), out, nil
